...
${category ? `CATEGORÍA: ${category}` : ''}`;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        if (!isTransient || attempt === maxAttempts) {
          break; // Fall through to local procedural fallback solver instead of hard failing
        }
    ...
    if (response && response.text) {
      try {
        ...
        console.warn('Failed to parse JSON from Gemini response, using fallback solver:', parseErr);
      }
    }

    if (!solutionData) {
      console.log('Using local fallback physics resolution engine due to model overload or invalid response.');
      solutionData = generateLocalFallbackSolution(problemStatement, category);
      solutionData.physicalDiscussion += ' (Nota: Resolución generada por el motor analítico local debido a alta demanda momentánea del servicio de IA).';
    }
    ...
    // Even on unexpected errors, return a robust fallback solution instead of failing the user
    const fallback = generateLocalFallbackSolution(req.body?.problemStatement, req.body?.category);
    fallback.physicalDiscussion += ' (Nota: Generado por motor de respaldo ante interrupción del servicio).';
